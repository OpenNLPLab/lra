...
class FourierMMLayer(nn.Module):
    def __init__(self,max_position_embeddings,hidden_size):
        super().__init__()
        self.dft_mat_seq = nn.Parameter(torch.tensor(linalg.dft(max_position_embeddings), requires_grad=False))
        self.dft_mat_hidden = nn.Parameter(torch.tensor(linalg.dft(hidden_size), requires_grad=False))
...
class FNetLayer(nn.Module):
    def __init__(self,
                ...
                dropout_rate=0.1):
        super().__init__()
        self.fft = FourierMMLayer(max_position_embeddings,hidden_size) if fourier == 'matmul' else FourierFFTLayer()
...
class FNetFairseqLayer(nn.Module):
    def __init__(self,
        ...
        self.fnet = FNetLayer(max_position_embeddings=max_pos_embeddings,
                hidden_size=d_model,
